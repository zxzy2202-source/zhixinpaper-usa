...
export default function WhatsAppButton() {
  ...
  return (
    <div
      className={`fixed bottom-4 right-3 md:bottom-6 md:right-6 z-50 flex flex-col items-end gap-3 transition-[opacity,transform] duration-500 ${
    ...
    >
      ...
      >
        <div className="relative bg-white  shadow-2xl border border-slate-100 px-4 py-3 max-w-[220px]">
      ...
      <a
        ...
        className="relative flex h-12 w-12 items-center justify-center rounded-full bg-[#25D366] shadow-lg shadow-green-500/40 transition-[background-color,box-shadow,transform] duration-200 hover:bg-[#20bd5a] hover:shadow-xl hover:shadow-green-500/60 active:scale-95 md:h-14 md:w-14"
      >
        {/* Pulse ring */}
        {pulse && (
          <>
            <span className="absolute inset-0 rounded-full bg-[#25D366] animate-ping opacity-30" />
            <span className="absolute inset-[-5px] rounded-full border-2 border-[#25D366]/40 animate-pulse md:inset-[-6px]" />
          </>
        )}
